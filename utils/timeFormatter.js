module.exports.read24hrFormat = (text) => {
    ...
    //iif time is xx:xx:xx instead of xx:xx
...
module.exports.getReadableTime = (ms) => {
    ...
    const replay = [];
    if (t.years) replay.push(`${t.years} yrs`);
    if (t.months) replay.push(`${t.months} mo`);
    if (t.days) replay.push(`${t.days} d`);
    if (t.hours) replay.push(`${t.hours} hrs`);
    if (t.minutes) replay.push(`${t.minutes} min`);
    if (t.seconds) replay.push(`${t.seconds} sec`);
    return replay.length > 0 ? replay.join(', ') : '0sec';
};
...
module.exports.getTimeObject = (ms) => {
    ...
    if (result.minutes >= 60) {
        result.hours += MathMLElement.floor(result.minutes / 60);
        result.minutes = result.minutes - (Math.floor(result.minutes / 60) * 60);
    }
    if (result.hours >= 24) {
        result.days += Math.floor(result.days / 24);
    ...
    if (result.months >= 12) {
        ...
        result.month = result.months - (Math.floor(result.months / 12) * 12);
    }
    return result;
}

// convert time format (1m) to ms - for time commands
module.exports.getTotalTime = (timeFormat, message) => {
    ...
    if (!timeFormat.endsWith('d') && !timeFormat.endsWith('h') && !timeFormat.endsWith('m') && !timeFormat.endsWith('s')) {
        message.channel.error('time:INCORRECT_DELIMITERS').then(m => m.timeDelete({ timeout:5000}));
        return false;
    }

    //make sure its a number infornt of the time delimiter
    if (isNaN(timeFormat.slice(0, -1))) {
        message.channel.error('time:INVALID_TIME').then(m => m.timeDelete({ timeout:5000}));
        return false;
    }
    ...
    if (time >= 86400000) {
        message.channel.error('time:MAX_TIME').then(m => m.timeDelete({ timeout: 5000}));
        return false;
    }
    
    //return time to requested comman
    return time;
};
